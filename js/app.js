...
function moveRover(currentDirection, currentPosition, actionDirection, actionPosition) {
    ...
    // Si los valores recibidos en la acción de posición son los correctos ejecuto la evalución
        ...
        switch (currentDirection) {
            ...
            case "N":
                if (nextCellForward != null && nextCellForward.getAttribute('data-access') == 1 && actionPosition == 1) {
                    if (currentPosition.y-1 < 0) {
                        errorMove("ERROR BACKWARD ROVER go over the grid");
                    } else {
                        console.log('N Forward Si');
                        rover.position.y = currentPosition.y-1;
                        updateRoverDOM(rover, 0);
                    }
                } else if (actionPosition == 2 && nextCellBackward != null && nextCellBackward.getAttribute('data-access') == 1){
                    if (currentPosition.y+1 < matrixGrid.yCells) {
                        console.log('N Back si');
                        rover.position.y = currentPosition.y+1;
                        updateRoverDOM(rover, 0);
                    } else {
...
function loadTerrain() {
    let terrainDOM = document.getElementById('terrain');
...
// Defino la variable oldCellForward de forma global en el alcance para que almacene el valor de la anterior posición activa y la actualize cuando haya una nueva celda 
let oldCellForward;
let oldCellBackward;
...
function currentsCellsDOM(currentRover) {
    ...
    if (oldCellForward != null) {
        if (oldCellForward.getAttribute('data-access') == 1) {
            ...
            oldCellForward.style.backgroundColor = cellProperties.bgNoAccess;    
        }
    }
    ...
    if (nextCellForward != null) {
        if (nextCellForward.getAttribute('data-access') == 1) {
            nextCellForward.style.backgroundColor = 'green';
        } else {
            nextCellForward.style.backgroundColor = 'red';    
        }
    ...
    if (oldCellBackward != null) {
        if (oldCellBackward.getAttribute('data-access') == 1) {
            ...
            oldCellBackward.style.backgroundColor = cellProperties.bgNoAccess;    
        }
    }
    // Cambiamos el elemento del DOM para ver si se puede mover o no
    if (nextCellBackward != null) {
        if (nextCellBackward.getAttribute('data-access') == 1) {
            nextCellBackward.style.backgroundColor = 'purple';
        } else {
            nextCellBackward.style.backgroundColor = 'pink';    
        }
